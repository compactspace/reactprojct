...
import { UpdateClassInfoModalStyle } from "../createclassComponent/modalStyle/UpdateClassInfoModalStyle";
...
export const MyInfoWrapper = styled.div`
  ...
  // select 와 .selectLable 의 폰트와 라인헤이트는 한쌍으로 기억해라 수동 보정을 해야함...
  & select {
    display: block;
    font-size: 20px;
  }
...
export const 미리보기모달레퍼 = styled.div`
  ///흠.. 아무튼 이상한데
...
export const UpdateClassInfoModal = () => {
  let IP;
  ...
  const [수업정보상태가변경됬는지, set수업정보상태가변경됬는지] =
    useState(false);
  const isFirstRender = useRef(true);
  const [기본클래스정보모달열기, set기본클래스정보모달열기] = useState(false);

  const [사진배열상태가변경됬는지, set사진배열상태가변경됬는지] =
    useState(false);
  const [사진배열, set사진배열] = useState([]);

  const [onedayClassNumList, setOnedayClassNumList] = useState([]);

  const [onedayClassNumCnt, setOnedayClassNumCnt] = useState(0);
  ...
  useEffect(() => {
    axios
      .post(`http://${IP}:4000/teacher/getRocoredOnedayclassNumList`, {
        headers: {
      ...
      .then((res) => {
        let { onedayClassNumList, onedayClassNumCnt } = res.data;
        setOnedayClassNumList(onedayClassNumList);
        setOnedayClassNumCnt(onedayClassNumCnt);
        const box = { ...searchKeyword };
        box.onedayclass_num = onedayClassNumList[0].onedayclass_num;
  ...
  useEffect(() => {
    if (onedayClassNumCnt === 0) return;
    if (searchKeyword.onedayclass_num === "") return;

    axios
      .post(
        `http://${IP}:4000/teacher/getSelectOneOnecayClassInfo`,
        searchKeyword
      )
      .then((res) => {
        let { selectOneonedayClassInfo } = res.data;
        // alert(JSON.stringify(selectOneonedayClassInfo));
        set수업정보상태(selectOneonedayClassInfo[0]);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  useEffect(() => {
    //바로위 useEffect의 시간차로 조건 추가
    if (수업정보상태 === undefined) {
      return;
    }
    //바로위 useEffect에서의 set수업정보상태 가 저장된 이후 false로 바꾼다.
    if (isFirstRender.current) {
      isFirstRender.current = false;
      return;
    }
    //이제 부터 이벤트 핸들러(바로 밑) 에서 수업정보상태 가 변경된다.
    console.log("난 뜨면안됨 상태가 변경되어야만 떠야함");
    set수업정보상태가변경됬는지(true);
  }, [수업정보상태]);
  ...
  const handleConfirm = async () => {
    ...
    // 그냥 일관성이 있게, 사진만 변경되어도 레코드에 그냥 삽입하도록 하자.

    box.reserve_img = 사진배열;
    ...
    console.log(box);
    await axios
      .post(`http://${IP}:4000/teacher/updateclassinfo`, box, { headers })
      .then((res) => {
  ...
  return (
    <>
      <div className="length" style={{ width: "100%" }}>
        {onedayClassNumCnt > 0 ? (
          <>
            <검색조건레퍼>
              <select id="select" onChange={onedayclassNumHandler}>
                {onedayClassNumList.map((item) => {
                  return (
                    <>
                      <option
                        key={item.onedayclass_num}
                        data-var={item.onedayclass_num}
                      >
                      ...
                      <div className="read writie">
                        <div className="filebox">
                          ...
                          <input
                            ...
                            multiple
                            onChange={(e) =>
                              handleImageUpload(e.target.files[0], e)
                            }
                          ></input>
                        </div>
                        <div className="filebox">
            ...
            <CustomConfirmModal>
              <div className="modal-overlay">
                <div className="modal-content">
                  <p>클래스 정보 {message} 안내</p>
                  <p className="updateAgree">
                    클래스정보의 수정시 가격 정보 와 할인 계산은 수정 시간
                    기준으로 산정됩니다.
                  </p>
                  <p className="updateAgree">
                    이미 계산을 하여 환불을 하거나, 현재 계산을 진행중인
                    소비자들에게 적용되지 않습니다. 동의하십니까?
                  </p>
...
const CustomConfirmModal = styled.div`
  .modal-overlay {
    position: fixed;
    top: 0;
    left: 0;
