...
export const ReservedModal = (props) => {
  ...
  const Payment = () => {
    ...
    makeMerchantUid =
      `${process.env.REACT_APP_IMPORT_KEY}` +
      hours +
      minutes +
      seconds +
      milliseconds;
    ...
    IMP.request_pay(
      ...
      async function (rsp) {
        if (rsp.success) {
          await axios
            .post(`http://${IP}:4000/user/payment`, reserveinfodata)
            .then((result) => {
              let statusNum = result.data.StatusCode;
              let access_token = result.data.ImportAccessToken;
              console.log(
                "statusNum:  " + statusNum + " access_token: " + access_token
              );
              if (statusNum == 1) {
                alert("결제완료 ");
              ...
              if (statusNum == -1) {
                ...
                alert(
                  "예약 마감으로 환불처리 되었습니다. 다른 날짜를 선택하여주세요"
                );
                return;
              }
            })
  ...
  const TestPayment = async () => {
    ...
    await axios
      .post(`http://${IP}:4000/user/testpayment`, reserveinfodata)
      .then((result) => {
        ...
        <div className="modalbox">
          ...
          <div className="detaile">
            <div className="detaileinfo">
              ...
              <span className="info">
                <input
                  ...
                  type="text"
                  defaultValue="예약자명을 입력해주세요"
                ></input>
              </span>
            </div>
            <div className="detaileinfo">
              <span className="titleinfo">예약연락쳐</span>
              <span className="info">
                <input
                  id="reserve_tell"
                  className="inputDefault"
            ...
            <div className="detaileinfo">
              ...
              <span className="info">{props.fordbobj.onedayclass_useTime}</span>
            </div>
            <div className="detaileinfo">
              <span className="titleinfo">클래스이용비용</span>
              <span className="info">{props.fordbobj.onedayclass_price}</span>
            </div>
            <div className="confirm">
              ...
              <span onClick={TestPayment}>테스트결제</span>
            </div>
          </div>
        </div>
      </Modalwarperr>
