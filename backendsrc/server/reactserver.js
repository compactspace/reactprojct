...
//테스트 로그인
app.post("/testlogin2", (req, res) => {
  let { id, password } = req.body;

  console.log("포스트매핑 id: " + id);
...
app.use("/transaction2", async (req, res) => {
  ...
  try {
    await con.beginTransaction();
    let sql =
      "select reserved_amount ,total_amount from ticket where ticket_id=1";
    executequery = await con.query(sql);
...
app.use("/transaction3", async (req, res) => {
  ...
  try {
    await con.beginTransaction();
    let sql =
      "select reserved_amount ,total_amount from ticket where ticket_id=1 for update";
    executequery = await con.query(sql);
    result1 = executequery[0][0].total_amount;
    result2 = executequery[0][0].reserved_amount;
